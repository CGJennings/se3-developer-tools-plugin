...
/**
 * A tracked window proxy that creates its target window on demand. The
 * window is created automatically using the supplied window title as the basis
 * for the name of the Panel class to load for the window content.
 * 
...
class DevToolProxy extends TrackedWindowProxy {
	...
	/**
	 * Called when unloading the plug-in to allow it to perform any needed
	 * final unload.
	 */
	public void unload() {
		if( tw != null ) {
